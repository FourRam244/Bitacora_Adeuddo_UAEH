...
class CancelarVale:
    def __init__(self,master):
        ...
        # Título del menú
        titulo_label = tk.Label(self.root, text="Cancelar Vale", font=("Helvetica", 20))
        ...
        guardar_button.grid(row=5, column=0, sticky='nsew')
        # Botón para guardar los datos
        cancelar_button = tk.Button(self.root, text="Cancelar",command=self.cancelar_consulta)  # Vincula el botón al método consultar_vale
        cancelar_button.grid(row=5, column=1, sticky='nsew')
        
        # Label para mostrar la imagen del vale
        self.vale_image_label = tk.Label(self.root)
        self.vale_image_label.grid(row=6, columnspan=2)
